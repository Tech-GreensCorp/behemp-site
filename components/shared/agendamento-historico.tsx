...
import { AgendamentoDetalheDialog } from '@/components/shared/agendamento-detalhe-dialog';
...
export const STATUS_LABEL: Record<HistoricoAgendamentoItem['status'], string> = {
...
export const STATUS_CLASSE: Record<HistoricoAgendamentoItem['status'], string> = {
 ...
 * agendamento, independente da etapa do wizard. Cada linha abre um detalhe com as ações
 * disponíveis (cancelar reserva pendente, ou remarcar) em `AgendamentoDetalheDialog`.
 */
export function AgendamentoHistorico({ items }: AgendamentoHistoricoProps) {
  const [itemAberto, setItemAberto] = useState<HistoricoAgendamentoItem | null>(null);
    ...
    <>
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className="flex w-full items-center gap-3 rounded-lg border border-border/50 p-3 text-left transition-colors hover:border-primary/40 hover:bg-primary/[0.03]"
              >
      ...
      {itemAberto && (
        <AgendamentoDetalheDialog
          ...
          onOpenChange={(aberto) => !aberto && setItemAberto(null)}
        />
      )}
    </>
  );
