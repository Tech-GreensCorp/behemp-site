...
export function AdminSidebar() {
  ...
  function renderBadge(href: string, isCollapsed: boolean) {
    if (href === '/admin/mensagens' && naoLidas > 0) {
      return (
        <span className={cn(
          'flex items-center justify-center rounded-full bg-white font-bold text-primary shadow-sm transition-all',
          isCollapsed
            ? 'absolute -right-1 -top-1 h-3 w-3'
            : 'right-3 h-5 min-w-5 text-[10px] px-1'
        )}>
          {isCollapsed ? (
            <span className="h-1.5 w-1.5 animate-ping rounded-full bg-primary/70" />
          ) : (
    ...
    if (href === '/admin/atribuir-medico' && semMedico > 0) {
      return (
        <span className={cn(
          'flex items-center justify-center rounded-full bg-amber-400 font-bold text-amber-950 shadow-sm transition-all',
          isCollapsed
            ? 'absolute -right-1 -top-1 h-3 w-3'
            : 'right-3 h-5 min-w-5 text-[10px] px-1'
        )}>
        ...
        >
          {!collapsed && (
            <Link href="/admin" className="flex items-center gap-2 overflow-hidden shrink-0 animate-fade-in">
              ...
              />
              <span className="text-[9px] font-bold uppercase tracking-widest text-white bg-white/15 px-1.5 py-0.5 rounded-md shrink-0">
                Admin
              </span>
            </Link>
          )}
