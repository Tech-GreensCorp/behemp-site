...
**09/09/2026 — 480 casos em 14 guardas.** (Eram 199 em 8 na medição de 24/08, quando três
...
| `handoff-do-parceiro-e-assinado-e-idempotente` | a assinatura deixar de cobrir corpo ou id, a janela de 300 s sumir, a rota ler `json()` antes de verificar, o corpo voltar a sobrescrever o id do evento, o índice do evento virar `unique` (reentrega passaria a dar 500), ou `/api/parceiros` sair das rotas públicas — **29 casos**. A idempotência aqui decide **gateway e desconto** do lado da Greens | `pnpm test`                          |
