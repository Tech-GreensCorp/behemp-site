...
const TIPO_COLORS: Record<string, string> = {
  ...
  estavel: 'bg-amber-100 text-amber-800',
...
const QUALIDADE_LABELS: Record<string, string> = { ruim: 'Ruim', regular: 'Regular', boa: 'Boa', excelente: 'Excelente' };
...
export function TabEvolucao({ pacienteId }: TabEvolucaoProps) {
  ...
  async function handleSalvar() {
    if (!form.data || !form.tipo || !form.conteudo) {
      toast.error('Preencha os campos obrigatórios: Data, Tipo e Observações');
      return;
    }
    setSalvando(true);
  ...
  return (
    <div className="space-y-6">
      ...
      {mostrarForm && (
        <Card className="border-border/40 shadow-sm">
          <CardHeader className="border-b border-border/30 pb-4">
            <CardTitle className="font-heading text-base">Nova Evolução</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 pt-6">
            ...
            <div>
              ...
              <Textarea value={form.sintomasAtuais} onChange={e => u('sintomasAtuais', e.target.value)} rows={2} />
            </div>

            <div>
              <Label>Efeitos Colaterais</Label>
              <Textarea value={form.efeitosColaterais} onChange={e => u('efeitosColaterais', e.target.value)} rows={2} />
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div>
                ...
                <Select value={form.qualidadeSono} onValueChange={v => u('qualidadeSono', v)}>
                  <SelectTrigger>
                    <SelectValue placeholder="Selecione">
                      {form.qualidadeSono ? (QUALIDADE_LABELS[form.qualidadeSono] ?? form.qualidadeSono) : undefined}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Select value={form.bemEstar} onValueChange={v => u('bemEstar', v)}>
                  <SelectTrigger>
                    <SelectValue placeholder="Selecione">
                      {form.bemEstar ? (QUALIDADE_LABELS[form.bemEstar] ?? form.bemEstar) : undefined}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="ruim">Ruim</SelectItem>
            ...
            <div>
              ...
              <Textarea value={form.conteudo} onChange={e => u('conteudo', e.target.value)} rows={3} />
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <Button
                ...
                disabled={salvando || !form.data || !form.tipo || !form.conteudo}
      ...
      {/* Lista de evoluções */}
      {evolucoes.length === 0 && !mostrarForm ? (
        <Card className="border-border/40 shadow-sm">
            ...
            <CardContent className="p-5">
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${TIPO_COLORS[ev.tipo] ?? ''}`}>
                    {TIPO_LABELS[ev.tipo] ?? ev.tipo}
                  </span>
                  <p className="text-xs text-muted-foreground">
                    {new Date(ev.data + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}
                  </p>
                </div>
              ...
              <div className="mt-2 flex gap-1.5">
                {ev.qualidadeSono && <Badge variant="secondary">Sono: {QUALIDADE_LABELS[ev.qualidadeSono] ?? ev.qualidadeSono}</Badge>}
                {ev.bemEstar && <Badge variant="secondary">Bem-estar: {QUALIDADE_LABELS[ev.bemEstar] ?? ev.bemEstar}</Badge>}
              </div>
            </CardContent>
          </Card>
        ))
