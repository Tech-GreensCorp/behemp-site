...
export function ProdutoCard({ produto }: { produto: Produto }) {
  return (
    <Link href={`/paciente/farmacia/produtos/${produto.id}`} className="group block h-full">
      <Card className="h-full overflow-hidden rounded-2xl border border-border/20 bg-white p-0 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
